namespace BookieWookie.API.Services
{
    ...
    public class UserService : IUserService
    {
        ...
        public async Task<JWTTokenResponse?> Authenticate(AuthenticateRequest model)
        {
            ...
            User? user;
            user = await this.context.Users.SingleOrDefaultAsync(x => x.Username.ToLower() == model.Username.ToLower());
        ...
        public JWTTokenResponse CreateUser(CreateUserRequest model)
        {
            ...
            if (this.context.Users.Where(u => u.Username == model.Username).Count() > 0)
            {
        ...
        public User UpdateUser(UpdateUserRequest model, int userId)
        {
            ...
            var user = new User();
            user = this.context.Users.SingleOrDefault(u => u.UserId == model.UserId);
            if (user == null)
            {
        ...
        public User DeleteUser(int id, int userId)
        {
            ...
            var user = new User();
            user = this.context.Users.SingleOrDefault(u => u.UserId == id);
            if (user == null)
            {
